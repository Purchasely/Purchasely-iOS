...
typedef SWIFT_ENUM(NSInteger, PLYEvent, open) {
  ...
  PLYEventSubscriptionsTransferred = 35,
  PLYEventUserLoggedIn = 36,
  PLYEventUserLoggedOut = 37,
  PLYEventPresentationClosed = 38,
  PLYEventSubscriptionContentUsed = 39,
  PLYEventOptionsSelected = 40,
  PLYEventOptionsValidated = 41,
  PLYEventWebCheckoutError = 42,
  PLYEventWebCheckoutOpenedInWebBrowser = 43,
  PLYEventWebCheckoutTapped = 44,
  PLYEventWebCheckoutTimedOut = 45,
  PLYEventRedemptionConsumed = 46,
  PLYEventRedemptionFailed = 47,
};
...
/// The Web2App redemption a <code>REDEMPTION_CONSUMED</code> / <code>REDEMPTION_FAILED</code> reports on (MOB-347).
...
/// What a web redemption granted.
/// Every field is nullable on purpose: a redemption grants a subscription today, and this type is
/// the seam for whatever it may grant later.
SWIFT_CLASS("_TtC10Purchasely23PLYWebRedemptionContext")
@interface PLYWebRedemptionContext : NSObject
/// The redeemed subscription, or <code>nil</code> when the response carried none.
@property (nonatomic, readonly, strong) PLYSubscription * _Nullable subscription;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

@class PLYWebRedemptionResult;
/// Notified when a Web2App redemption settles, so the app can react to the grant itself rather
/// than by listening for the <code>REDEMPTION_CONSUMED</code> analytics event.
/// Register it on the start chain — <code>Purchasely.apiKey(…).webRedemptionDelegate(self).start()</code>.
/// There is deliberately no runtime setter: a redemption can settle during <code>start()</code> (a cold start
/// triggered by the link itself, or a pending token resumed from a previous launch), so a delegate
/// assigned afterwards would miss exactly the case it is most needed for.
/// Called on the <em>main thread</em>, once per settled redemption — success and failure alike, and
/// always <em>after</em> the matching <code>REDEMPTION_CONSUMED</code> / <code>REDEMPTION_FAILED</code> event has been handed to
/// a <code>PLYEventDelegate</code>. The two channels report the same settlement, so they never disagree about
/// the order it happened in.
/// <em>When</em> it is called is governed by <code>appHandlesRedemptionAlert</code> on the registration:
/// <ul>
///   <li>
///     <code>webRedemptionDelegate(self)</code> — the default. The SDK presents its success/failure popin and
///     calls you once the user <em>acknowledges</em> it, so you act on a screen they have already
///     dismissed. That popin is deliberately not routed through <code>PLYUIHandler</code>: it is what produces
///     the acknowledgment, so it cannot be intercepted and replaced.
///   </li>
///   <li>
///     <code>webRedemptionDelegate(self, appHandlesRedemptionAlert: true)</code> — no popin, and you are called
///     as soon as the redemption settles. Use it when the app owns the whole post-redemption
///     experience; <code>errorMessage</code> is then your only source for the expired link’s email hint.
///   </li>
/// </ul>
/// Exactly one call per settled redemption either way — including when the popin is dismissed for
/// the user because another redemption link arrived before they tapped OK.
SWIFT_PROTOCOL("_TtP10Purchasely24PLYWebRedemptionDelegate_")
@protocol PLYWebRedemptionDelegate
/// \param result the outcome. Check <code>isSuccess</code>, or <code>asResult()</code> from Swift.
///
- (void)webRedemptionCompletedWithResult:(PLYWebRedemptionResult * _Nonnull)result;
@end

/// Outcome of a Web2App redemption (<code>{scheme}://ply/redeem/{token}</code>), delivered to
/// <code>PLYWebRedemptionDelegate</code>.
/// Read <code>isSuccess</code> first: it decides which of the two field groups is populated. The flat shape is
/// what <code>@objc</code> can express — Swift callers can switch exhaustively via <code>asResult()</code>.
SWIFT_CLASS("_TtC10Purchasely22PLYWebRedemptionResult")
@interface PLYWebRedemptionResult : NSObject
/// <code>true</code> for a granted redemption, <code>false</code> for a failed one.
@property (nonatomic, readonly) BOOL isSuccess;
/// What the redemption granted. <code>nil</code> on failure, and nullable on success for a future
/// response shape that describes nothing.
@property (nonatomic, readonly, strong) PLYWebRedemptionContext * _Nullable context;
/// <code>true</code> when the token had already been redeemed. Always <code>false</code> on failure.
@property (nonatomic, readonly) BOOL replay;
/// Backend <code>error.code</code>. <code>nil</code> on success, and <code>nil</code> on a failure that never reached the
/// server (a transport error produces no code).
@property (nonatomic, readonly, copy) NSString * _Nullable errorCode;
/// Human-readable reason. <code>nil</code> on success. Never contains the token — a bearer credential has
/// no business crossing into app code.
/// For an expired link it <em>does</em> carry the backend’s masked email hint (“A new link was sent
/// to j***@example.com.”), which the <code>REDEMPTION_FAILED</code> event deliberately drops: the event
/// leaves the device, this does not. It is the only copy of that hint a host running
/// <code>appHandlesRedemptionAlert: true</code> will ever see, since the alert that otherwise carries it is
/// suppressed. English, and the hint is absent when the backend omitted it.
...
/// (core/src/main/java/io/purchasely/ext/Enums.kt <code>Attribute</code>). Order verified case-by-case as of
/// the v6 parity audit — <code>oneSignalPlayerId</code> was removed (iOS-only, no Android counterpart; use
/// <code>oneSignalExternalId</code> / <code>oneSignalUserId</code> instead), restoring strict 1:1 ordinal alignment.
...
typedef SWIFT_ENUM(NSInteger, PLYAttribute, open) {
  ...
  PLYAttributeMixpanelDistinctId = 6,
  PLYAttributeClevertapId = 7,
  PLYAttributeSendinblueUserEmail = 8,
  PLYAttributeIterableUserEmail = 9,
  PLYAttributeIterableUserId = 10,
  PLYAttributeAtInternetIdClient = 11,
  PLYAttributeMParticleUserId = 12,
  PLYAttributeCustomerioUserId = 13,
  PLYAttributeCustomerioUserEmail = 14,
  PLYAttributeBranchUserDeveloperIdentity = 15,
  PLYAttributeAmplitudeUserId = 16,
  PLYAttributeAmplitudeDeviceId = 17,
  PLYAttributeMoengageUniqueId = 18,
  PLYAttributeOneSignalExternalId = 19,
  PLYAttributeBatchCustomUserId = 20,
  PLYAttributeOneSignalUserId = 21,
};
...
/// Sets the anonymous user id that the SDK reports for this device.
...
///
/// returns:
/// a new builder that carries the anonymous user id.
- (PurchaselyBuilder * _Nonnull)appAnonymousUserId:(NSUUID * _Nullable)value SWIFT_WARN_UNUSED_RESULT;
/// Sets the anonymous user id that the SDK reports for this device, and controls whether the
/// id replaces an id that already exists.
/// The SDK applies the id at <code>start()</code>, before the SDK sends a network request or an event.
/// When <code>override</code> is <code>false</code>, the SDK keeps a stored id and ignores <code>value</code>. When <code>override</code>
/// is <code>true</code>, the SDK replaces the stored id.
/// <em><code>override: true</code> splits the user history.</em> The backend keeps every event, purchase and
/// subscription that the backend recorded against the previous id. The SDK does not merge the
/// two identities, so this device loses access to the earlier history. The SDK logs a warning
...
/// returns:
/// a new builder that carries the anonymous user id.
- (PurchaselyBuilder * _Nonnull)appAnonymousUserId:(NSUUID * _Nullable)value override:(BOOL)override SWIFT_WARN_UNUSED_RESULT;
...
/// Routes Purchasely <em>API</em> traffic through Purchasely’s own proxy at
/// <code>https://svc.purchasely.io</code>, for regions where <code>api.purchasely.io</code> is unreachable
/// (mainland China). Equivalent to <code>proxy(api:)</code> with that host.
/// Only the API host moves: the paywall and tracking hosts are not affected by this
/// setting (they continue to follow <code>environment(_:)</code>).
/// Start-time only — the value is applied when the chain’s <code>start()</code> runs, and there is no
...
///
/// \endcode
- (PurchaselyBuilder * _Nonnull)proxy SWIFT_WARN_UNUSED_RESULT;
/// Routes Purchasely <em>API</em> traffic through <code>api</code> instead of <code>api.purchasely.io</code>, for
/// regions where that host is unreachable (mainland China). Purchasely operates a proxy at
/// <code>https://svc.purchasely.io</code>; you may also host your own.
/// Only the API host moves: the paywall and tracking hosts are not affected by this
/// setting (they continue to follow <code>environment(_:)</code>).
/// Must be an <code>https</code> base URL with a host, and carry no query, fragment or credentials.
/// Anything else is ignored with an <code>.error</code> log and the API host stays on production, so a
/// misconfiguration never breaks the SDK. A trailing slash is dropped — SDK resource paths
/// already lead with <code>/</code>.
/// Start-time only — the value is applied when the chain’s <code>start()</code> runs, and there is no
/// runtime setter for it.
/// \code
/// Purchasely
...
///     .start()
///
/// \endcodePass <code>nil</code> to explicitly turn the proxy off and restore <code>api.purchasely.io</code> — the
/// inverse of this modifier and of <code>proxy()</code>. A chain that never calls either one leaves
/// the current setting untouched.
/// \param api <code>https</code> base URL of the API proxy, or <code>nil</code> for no proxy.
///
- (PurchaselyBuilder * _Nonnull)proxyWithApi:(NSURL * _Nullable)api SWIFT_WARN_UNUSED_RESULT;
...
/// React to a Web2App redemption settling (<code>{scheme}://ply/redeem/{token}</code>) — the supported
/// alternative to listening for the <code>REDEMPTION_CONSUMED</code> analytics event.
/// The delegate is called on the main thread, once per settled redemption, success or failure,
/// and always after the matching <code>REDEMPTION_CONSUMED</code> / <code>REDEMPTION_FAILED</code> event reached a
/// <code>PLYEventDelegate</code> — it complements that event, it does not replace it. On success the result
/// carries a context holding the granted <code>PLYSubscription</code>, so the app can unlock content,
/// present a dedicated screen, or run a tailored onboarding.
/// \code
/// Purchasely.apiKey("…")
...
///         guard !replay else { return }          // already handled on an earlier launch
///         unlock(context?.subscription)
///     case .failure(let code, _):
///         showRetry(for: code)
///     }
/// }
///
/// \endcode<em>Only settable here.</em> There is no runtime setter, because a redemption can settle during
/// <code>start()</code> — a cold start triggered by the link itself, or a pending token resumed from a
/// previous launch — and a delegate assigned afterwards would miss exactly that case. The SDK
/// holds it weakly, so the host stays its owner.
/// \param value the delegate, or <code>nil</code> for none. Held weakly by the SDK.
///
/// \param appHandlesRedemptionAlert whether the <em>app</em> tells the user how the redemption
/// settled, instead of the SDK. Defaults to <code>false</code> — the SDK does it.
/// <ul>
///   <li>
///     <code>false</code> — the SDK presents the success/failure popin, and the delegate is called when
///     the user <em>acknowledges</em> it (taps OK), so the app acts on a screen the user has
///     already dismissed. That popin is not interceptable: unlike every other SDK alert it
///     does not go through <code>PLYUIHandler</code>, precisely so this acknowledgment always exists.
///   </li>
///   <li>
///     <code>true</code> — no popin. The delegate is called as soon as the redemption settles, right
///     behind its event, and the app owns the entire post-redemption experience. Note that
///     the expired-link guidance (“a new link was sent to j***@…”) then reaches you only
///     through <code>PLYWebRedemptionResult.errorMessage</code> — nothing else will show it.
///   </li>
/// </ul>
/// Either way the delegate is called exactly once per settled redemption. The in-progress
/// loader is unaffected — this flag governs the terminal popin only.
/// Objective-C callers must pass both arguments: Swift default arguments do not bridge.
///
- (PurchaselyBuilder * _Nonnull)webRedemptionDelegate:(id <PLYWebRedemptionDelegate> _Nullable)value appHandlesRedemptionAlert:(BOOL)appHandlesRedemptionAlert SWIFT_WARN_UNUSED_RESULT;
