...
/// Hides or shows the navigation bar of the SDK-created navigation controller hosting
/// this paywall. When the presentation is a flow, the setting applies to EVERY step of
/// the flow. Can be called before <code>display()</code> or while the paywall is on screen.
/// note:
/// This is a no-op when the paywall is pushed onto, or embedded in, a navigation
/// controller owned by your app — the SDK never modifies a navigation bar it does not own.
/// note:
/// Hiding the bar removes the bar title and the bar close button; the close
/// affordance is automatically rendered as an in-content button instead.
/// note:
/// Campaign-triggered paywalls never expose a <code>PLYPresentation</code>; use
/// <code>Purchasely.hideNavigationBar(_:)</code> for those.
/// \param hidden <code>true</code> to hide the navigation bar, <code>false</code> to show it.
///
/// \param animated whether a change applied to an on-screen paywall is animated.
///
...
/// Force-hides the navigation bar on every SDK-displayed paywall, current and future,
/// including campaign-triggered ones. Pass <code>false</code> to revert to the default behaviour.
/// Currently displayed paywalls are refreshed in place.
/// The SDK only ever hides navigation bars it created itself: a paywall pushed onto,
/// or embedded in, one of your app’s navigation controllers is never modified.
/// \param hide <code>true</code> to force-hide, <code>false</code> to revert.
///
